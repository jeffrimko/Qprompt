...
== qprompt-0.16.0 (TODO)
=== Added
  - When using `Menu.main(auto=True)` can now pass in `-d` as an argument to run the default menu entry.

=== Fixed
  - Fixed exception thrown when using `Menu.main(auto=True, loop=False)` and an invalid argument is passed in.
